<template>
  ...
  >
    <swiper-slide v-for="(x, index) in swiperData" :key="index">
      <master-card :showdata="x.product" />
      <p
        class="text-center relative-position"
        v-text="lang.lagn === 'en' ? x.product.name_en : x.product.name_ar"
      ></p>
      <p
        class="text-center relative-position"
        v-text="lang.lagn === 'en' ? x.product.price + ' Rs' : x.product.price + ' ريال  '"
      ></p>
      <p v-show="x.product.price > 500" class="">
